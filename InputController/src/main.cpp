...
InputController inputController("ArduinoMega", PLATFORM_WPC);
...
void setup() {
    ...
    Serial.println("Welcome to the Twilight Zone!");

    //inputController.switchMatrix()->start();
    //inputController.lightMatrix()->start();

    //inputController.pin2Dmd()->setDebug(true);
}
